...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const hamburgerAnimRef = useRef<gsap.core.Tween | null>(null);
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    if (opening) {
      // Transform to X
      hamburgerAnimRef.current = gsap.to([line1, line2, line3], {
        duration: 0.3,
        ease: 'power2.inOut',
        y: (i) => i === 0 ? 8 : i === 2 ? -8 : 0,
        rotate: (i) => i === 0 ? 45 : i === 2 ? -45 : 0,
        opacity: (i) => i === 1 ? 0 : 1,
        overwrite: 'auto'
      });
    } else {
    ...
    >
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <div className="sm-panel-inner">
          ...
          {customFooter && (
            <div className="sm-custom-footer">
              {customFooter}
            </div>
          )}
        </div>
